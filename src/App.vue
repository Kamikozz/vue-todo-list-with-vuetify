<template><div id="app"><v-app><v-content><v-container grid-list-md>
            <h1>{{ msg }}</h1>
            <v-layout row wrap>
              <v-flex offset-lg3 lg6 offset-xs2 xs8>
                  <v-text-field v-model="title" label="Enter your note here" clearable rows="1" textarea hide-details></v-text-field>
                <v-text-field v-show="ex4" v-model="details" label="Details" rows="2" clearable textarea hide-details></v-text-field>
                <v-checkbox class="pa-0" label="Add details" v-model="ex4" color="primary" hide-details></v-checkbox>
                  <v-card-text style=" position: relative" class="mt-3 ml-5">
                    <v-btn v-on:click="createNote()" absolute dark fab top right color="primary"><v-icon>add</v-icon></v-btn></v-card-text>
              </v-flex>
              <v-flex xs12 lg6>
            ...
            </v-layout></v-container></v-content></v-app></div></template>
